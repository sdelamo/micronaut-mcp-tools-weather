...
class ToolsCallGetWeatherForecastByLocationTest {

    @Test
    void toolsCallGetWeatherForecastByLocationTest() throws IOException, JSONException {
        try (EmbeddedServer mockServer = ApplicationContext.run(EmbeddedServer.class)) {
            ...
            try (EmbeddedServer server = ApplicationContext.run(EmbeddedServer.class, config)) {
                ...
                HttpRequest<?> request = HttpRequest.POST("/mcp", readResource(resourceLoader, "classpath:toolCallGetWeatherForecastByLocation.json").orElseThrow());
                String json = assertDoesNotThrow(() -> client.retrieve(request));
                assertNotNull(json);
                String expectedJson = readResource(resourceLoader, "classpath:toolCallGetWeatherForecastByLocationResponse.json").orElseThrow();
